...
function onMouseOverWithDelay(cm, e) {
    const state = cm.state.textHover, delay = state.options.delay;
    clearTimeout(state.timeout);
    if (e.srcElement) {
        // hack for IE, because e.srcElement failed when it is used in the
        // tiemout function
        e = {
...
function onMouseOver(cm, e) {
    ...
    if (node) {
        const state   = cm.state.textHover, data = getTokenAndPosAt(cm, e);
...
function optionHandler(cm, val, old) {
    ...
    if (val) {
        const state = cm.state.textHover =
            new TextHoverState(cm, parseOptions(cm,
                val));
...
export default function (cm) {
    cm.defineOption("textHover", false, optionHandler); // deprecated
    return cm;
}
