...
function cwd() {
    return path.resolve.apply(path, [process.env.SUBSCHEMA_PROJECT_DIR || process.cwd()].concat(asArray(arguments)));
}
function pkg() {
...
function dependency(current) {
    ...
    return path.resolve.apply(path, [cwd(), 'node_modules'].concat(asArray(arguments)));
}
function re(str) {
...
function keys(obj) {
    ...
    for (var i = 0, l = arguments.length; i < l; i++) {
        var c = arguments[i];
        if (c) Object.keys(c).reduce(keys$unique, ret);
    }
...
function _filteredDependencies(userPkg = pkg(), includes, excludes = [DEFAULT_EXCLUDE]) {

    var all = [userPkg.name].concat(keys(userPkg.dependencies, userPkg.devDependencies, userPkg.peerDependencies));
    if (!all) {
        return [];
    }
...
function useAlias(alias = {}) {
    ...
    if (process.env.SUBSCHEMA_USE_ALIASES) {
        process.env.SUBSCHEMA_USE_ALIASES.split(/,\s*/).forEach(parseAlias, alias);
    }
    return alias;
}
...
function useExternals(externals = {}) {
    if (process.env.SUBSCHEMA_USE_EXTERNALS) {
        return process.env.SUBSCHEMA_USE_EXTERNALS.split(/,\s*/).reduce(function (ret, key) {
...
function useCustomConf(customConf, confFile = SUBSCHEMA_CONF, deps = pkg()) {
    aliasDependencies(deps).forEach(function (key) {
        if (fs.existsSync(dependency(key, confFile))) {
            customConf = applyFuncs(customConf, require(dependency(key, confFile)));
...
function useDepAlias(alias = {}, deps = pkg()) {
    ...
    if (process.env.SUBSCHEMA_NO_DEPENDENCY_ALIASES) return alias;
    if (!process.env.SUBSCHEMA_DEPENDENCY_ALIASES) {
        aliasArr = aliasDependencies(deps);
    } else {
        aliasArr = unique([deps.name], process.env.SUBSCHEMA_DEPENDENCY_ALIASES.split(/,\s*/));
    }
    return aliasArr.filter(hasSource).reduce(function (ret, key) {
        ret[key + '/lib/style.css'] = dependency(key, 'lib', 'style.css');
        ret[key] = ret[key + '/lib'] = dependency(key, 'src');
        return ret;
    }, alias);
}
