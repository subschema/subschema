<script lang="ts">
  import { Form, FormProvider, Button, createDefaultContainer, createFormContainer } from '@subschema/svelte';
  import type { ExampleDef } from './examples';
  import StepPanel from './StepPanel.svelte';
  import UpperCaseTextField from './UpperCaseTextField.svelte';
  ...
  const presetContainer = createFormContainer(base, { types: { Text: UpperCaseTextField } });
...
<div class="space-y-8">
  <div>
    ...
    <p class="mt-2 text-sm text-gray-500 dark:text-gray-500">
      This example uses
      <code class="rounded bg-gray-100 px-1 dark:bg-gray-800">{'<FormProvider container={preset}>'}</code>
      to override the <code class="rounded bg-gray-100 px-1 dark:bg-gray-800">Text</code>
  ...
  <div>
    ...
    <div class="grid grid-cols-1 items-start gap-4 lg:grid-cols-[1fr_auto_1fr_auto_1fr]">
      ...
      <StepPanel step={2} title="JSON Schema" subtitle="Compiled output" color="indigo">
        {#snippet children()}
          <div class="relative">
            <button onclick={copySchema} class="absolute top-2 right-2 rounded bg-gray-100 px-2 py-1 text-xs transition hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700">
              {copied ? '✓ Copied' : 'Copy'}
            </button>
